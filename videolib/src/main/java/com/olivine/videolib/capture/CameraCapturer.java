...
import com.olivine.videolib.utils.Logging;
...
public abstract class CameraCapturer implements CameraVideoCapturer {
    ...
    public void startCapture(int width, int height, int framerate) {
        startCapture(width, height, framerate, false);
    }

    @Override
    public void startCapture(int width, int height, int framerate, boolean fixCam) {
            ...
            cameraThreadHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    createCameraSession(createSessionCallback, cameraSessionEventsHandler, applicationContext,
                            surfaceHelper, cameraName, width, height, framerate, fixCam);
                }
            }, delayMs);
        }
    }
        ...
        synchronized (stateLock) {
            ...
            if (deviceNames.length > 2) {
                if (cameraEnumerator.isFrontFacing(cameraName)) {
                    ...
                    Logging.d(TAG, "switchCamera, cameraName = " + cameraName);
                } else {
                    for (String name : deviceNames) {
                        if (name != null && cameraEnumerator.isFrontFacing(name)) {
                            cameraName = name;
                        }
                    }
                }
    ...
    abstract protected void createCameraSession(
            ...
            int width, int height, int framerate, boolean fixCam);
